...
function render() {
  ...
  if (!filtered.length) {
    ...
    td.textContent = "Keine Artikel gefunden.";
    tr.appendChild(td);
    body.appendChild(tr);
    return;
  }
...
function openDialog(id) {
  ...
  const flag = document.getElementById("item-flagged");
  ...
  flag.checked = item?.flagged || false;
...
function saveItem() {
  ...
  const loc = document.getElementById("item-location").value.trim();
  const date = document.getElementById("item-date").value;
  const note = document.getElementById("item-note").value.trim();
  const flag = document.getElementById("item-flagged").checked;
